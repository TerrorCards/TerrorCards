...
import { IonContent, IonHeader, IonPage, IonTitle, IonToolbar, IonSegment, IonSegmentButton, IonLabel,
  ...
  IonButton, IonSlides, IonSlide, IonImg, IonGrid, IonRow, IonCol,
  withIonLifeCycle
} from '@ionic/react';
...
class StoreContainer extends React.Component<props, state> {
  ...
  pullPacks =() => {
    callServer("packs","",this.props.user.ID)?.then((resp)=>{ return resp.json(); })
    .then((json)=>{ 
      console.log(json);
      if(json.length > 0) {
  ...
  pullInApp =() => {
    callServer("loadInAppItems","",this.props.user.ID)?.then((resp)=>{ return resp.json(); })
    .then((json)=>{ 
      console.log(json);
      if(json.length > 0) {
        ...
        items.push(
          <IonCard key={p.Name}>
            ...
            <IonCardContent>
              <IonGrid>
                <IonRow>
                  <IonCol><IonImg src={p.Image} /></IonCol>
                  <IonCol>
                    <div style={{display:'flex', flexDirection:"column"}}>
                      ...
                      <p></p>
                      <div style={{display:'flex', flex:2, justifyItems:'flex-end'}}>
                        <IonButton expand="block" onClick={() => {this._canBuy(p)}}>{p.Cost}</IonButton>
                      </div>
                    </div>
                  </IonCol>
                </IonRow>
          ...
          <IonCard key={"nopacks"}>
            <IonCardContent>
              <IonGrid>
                <IonRow>
                  <IonCol>
                    Make any coin purchase to see the special discounted packs here.
                  </IonCol>
                </IonRow>
              </IonGrid>
            </IonCardContent>
        ...
        items.push(
          <IonCard key={p.Name}>
            ...
            <IonCardContent>
              <IonGrid>
                <IonRow>
                  <IonCol><IonImg src={''} /></IonCol>
                  <IonCol>
                    <div style={{display:'flex', flexDirection:"column"}}>
                      <div style={{display:'flex', flex:2}}>{p.Desc}</div>
                      <div style={{display:'flex', justifyItems:'flex-end'}}>
                        <IonButton expand="block" onClick={() => {this._canBuy(p)}}>{p.Amount}</IonButton>
                      </div>
                    </div>
                  </IonCol>
                </IonRow>
  ...
  _canBuy =(p:any) => {
    ...
    //if(this.props.user.Credit >= p.Cost) {
      //call to pull packs.
      let packOrder = {'packID':p.ID,'packName':p.Name,'userID':this.props.user.ID, 'packSets': p.Set, 'packChase': p.Chase, 'packCost': p.Cost, 'packPer': p.PerPack};

      callServer("packsOrder",packOrder,this.props.user.ID)?.then((resp)=>{ return resp.json(); })
      .then((json)=>{ 
        console.log(json);
        if(json.length > 0) {
  ...
  render() {
    ...
    return (
        <IonContent>
          <IonSegment value={this.state.storeType} onIonChange={(e:any) => {this.changeStoreType(e.detail.value)}}>
