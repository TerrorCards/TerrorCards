...
import { IonContent, IonHeader, IonPage, IonTitle, IonToolbar, IonSegment, IonSegmentButton, IonLabel,
  ...
  IonButton, IonSlides, IonSlide, IonImg, IonGrid, IonRow, IonCol, IonIcon, IonPopover,
  withIonLifeCycle
} from '@ionic/react';
...
class FactoryContainer extends React.Component<props, state> {
  ...
  pullMelds =() => {
    callServer("pullFactoryList","",this.props.user.ID)?.then((resp)=>{ return resp.json(); })
    .then((json)=>{ 
      console.log(json);
      if(json.length > 0) {
  ...
  requestMeldAttempt =() => {
    callServer("meldFactoryItem", {MeldID:this.state.currentMeld.MeldID},this.props.user.ID)?.then((resp)=>{ return resp.json(); })
    .then((json)=>{ 
      console.log(json);
      if(json.length > 0) {
              ...
              <IonGrid>
                <IonRow>
                  <IonCol>
                    <IonGrid>
                        <IonRow>
                          <IonCol><IonImg src={p.MeldImage} /></IonCol> 
                        </IonRow> 
                        <IonRow>
                          <IonCol>Meld cards on the right to try and obtain the card above.</IonCol> 
                        </IonRow>                        
                        <IonRow>
                  ...
                  <IonCol>
                    <IonGrid>
                      <IonRow>
                        <IonCol><IonImg src={p.CardImage1} /></IonCol> 
                        <IonCol>
                      ...
                      {(p.CardImage2 !== null) && 
                        <IonRow>
                        <IonCol><IonImg src={p.CardImage2} /></IonCol> 
                        <IonCol>
                      ...
                      {(p.CardImage3 !== null) && 
                        <IonRow>
                        <IonCol><IonImg src={p.CardImage3} /></IonCol> 
                        <IonCol>
                      ...
                      {(p.CardImage4 !== null) && 
                        <IonRow>
                        <IonCol><IonImg src={p.CardImage4} /></IonCol> 
                        <IonCol>
                    ...
                    <IonButton disabled={!p.MeldMet} expand="block" onClick={() => {this._canMeld(p)}} color={(!p.MeldMet)?"danger":"success"}>{"Meld"}</IonButton>
                  </IonCol>
                </IonRow>
              </IonGrid>
            </IonCardContent>
  ...
  render() {
    return (
      ...
      <IonContent>
          ...
          onDidDismiss={() => this.setState({ showMeldAlert: false, event: undefined })}
          >
            Attempt the meld?
            <IonItem>
              <IonButton onClick={() => {
