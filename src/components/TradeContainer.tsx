...
class TradeContainer extends React.Component<props, state> {
  ...
  showAlertPrompt =(action:string, tradeID: any) => {
    this.setState({showAlert:true, alertType:action, activeTradeID: tradeID});
  }
  ...
  pullTrades =(user:string) => {
    return new Promise ((resolve:any, reject:any) => {
      callServer("showTrades",this.state.tradeStatus,user)?.then((resp)=>{ return resp.json(); })
      .then((json)=>{ 
        console.log(json);
        if(json.length > 0) {
          resolve(json);
  ...
  pullTradesMessages =(user:string, tradeId:string) => {
    return new Promise ((resolve:any, reject:any) => {
      callServer("requestTradeMessages",tradeId,user)?.then((resp)=>{ return resp.json(); })
      .then((json)=>{ 
        if(json.length > 0) {
          resolve(json);
        } else {
  ...
  cancelTradeRequest =(user:string, tradeId:string) => {
    return new Promise ((resolve:any, reject:any) => {
      callServer("cancelTrade",tradeId,user)?.then((resp)=>{ return resp.json(); })
      .then((json)=>{ 
        if(json) {
  ...
  executeTradeRequest =(user:string, tradeId:string) => {
    return new Promise ((resolve:any, reject:any) => {
      callServer("executeTrade",tradeId,user)?.then((resp)=>{ return resp.json(); })
      .then((json)=>{ 
        if(json) {
  ...
  pullCardDetails =(user:string, card: any) => {
    return new Promise ((resolve:any, reject:any) => {
      callServer("cardDetail",{number: card.Number, year:card.CardYear},user)?.then((resp)=>{ return resp.json(); })
      .then((json)=>{ 
        console.log(json);
        if(json) {
          resolve(json);
  ...
  render() {
    ...
    return (
        <IonContent>
          ...
          <IonSegment value={this.state.tradeStatus} onIonChange={(e:any) => {this.changeTradeTab(e.detail.value)}}  color="dark">
              ...
              },
              {
                ...
                handler: () => {
                  if(this.state.alertType === "cancel") {
                    this.cancelTradeRequest(this.props.user.ID, this.state.activeTradeID);
                    this.setState({showAlert:false, alertType:''});
                  } else if (this.state.alertType === "accept") {
                    this.executeTradeRequest(this.props.user.ID, this.state.activeTradeID);
                    this.setState({showAlert:false, alertType:''});
                  } else {
            ...
            <IonGrid>
              ...
              <IonCol>
                <IonItem>
                  ...
                  <IonTextarea placeholder="type here" rows={2} value={this.state.messageText} onIonChange={
                    (e:any)=> {this.updateTextMessage(e)}}
                  ></IonTextarea>
                  <IonButton color="success" expand="block" onClick={() => {this.sendMessage()}}>Send Message</IonButton>
                </IonItem>
              </IonCol>
              </IonRow> 
            </IonGrid>            
    ...
    for(let key in this.state.filteredTradeList) {
      ...
      this.state.filteredTradeList[key].map((card:any) => {
        ...
        if(card.UserID === this.props.user.ID) {
          yourItems.push(
            <div key={key+card.ID} style={{width:"20%", position:"relative"}}>
            <img src={card.Image} width="100%" onClick={()=>{this.showCardetails(card)}} />
            {(card.Count !== null && card.Count > 1) && <IonBadge class="quantity-badge">{card.Count}</IonBadge>}
            {(card.OwnedCount !== null && card.OwnedCount > 1) && <IonBadge class="quantity-owned-badge">{card.OwnedCount}</IonBadge>}
            </div>
          );
        } else {
          othersItems.push(
      ...
      tradeBlock.push(
            ...
            <IonCardHeader>
              <IonCardTitle>
                <div style={{display:'flex', flexDirection:"column"}}>
                  ...
                  <div style={{display:'flex', fontSize:'small', flexDirection:"row"}}>
                    <div>Ends</div>
                    <div style={{paddingLeft:5, paddingRight:5}}>{this.endDate(this.state.filteredTradeList[key][0].TradeDate)}</div>
                  </div>
                </div>
              </IonCardTitle>
            </IonCardHeader>          
        ...
        <IonRow>
            {(yourItems.length <= 0)?'Invalid trade':<div style={{display:'flex', flexDirection: "row"}}>{yourItems}</div>}
        </IonRow>
        <IonRow>
        ...
        <IonRow>
            {(this.state.tradeStatus === 'PENDING') && (this.state.filteredTradeList[key][0].TradeOwner !== this.props.user.ID) ? 
              <IonCol class="ion-text-center" size="4"><IonButton color="success" expand="block" onClick={() => {this.showAlertPrompt('accept', key)}}>Accept</IonButton></IonCol>
              :
            ...
            {(this.state.tradeStatus === 'PENDING')? 
              <IonCol class="ion-text-center" size="4"><IonButton color="danger" expand="block" onClick={() => {this.showAlertPrompt('cancel', key)}}>Cancel</IonButton></IonCol>  
                :
              <IonCol class="ion-text-center" size="4"></IonCol>
            }
        </IonRow>
  ...
  displayTradeMessages =(ID:any) => {
    ...
    this.pullTradesMessages(this.props.user.ID, ID).then((result: any) => {
      ...
      result.map((res:any, i:number) => {
        msgList.push(
          <IonRow key={res.ID+"-"+i}>
            <IonCol class={(res.Member === this.props.user.ID)?"ion-text-left":"ion-text-right"}>
              <div>{res.Message}</div>
              <div>{res.Member}</div>
            </IonCol>
          </IonRow>          
      ...
      this.setState({showInfoPopover: true, tradeMessages: msgList, activeTradeID: ID, messageText: ""})
    });
  }

  sendMessage =() => {
    return new Promise ((resolve:any, reject:any) => {
      callServer("appendTradeMessages",{tradeID: this.state.activeTradeID, message:this.state.messageText},this.props.user.ID)?.then((resp)=>{ return resp.json(); })
      .then((json)=>{ 
        if(json) {
  ...
  endDate = (tradeDate:string) => {
    const formatDate = new Date(new Date(tradeDate).getTime() + 60 * 60 * 24 * 1000);
    const fullDateTime = formatDate.toLocaleDateString() + " " + formatDate.toLocaleTimeString();
    return fullDateTime;
  }
